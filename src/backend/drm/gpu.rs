...
use super::device::{Device, DrmDevice};
use crate::backend::drm::{DrmOutputId, DrmRenderer};
use crate::backend::BackendHandler;
use eyre::Result;
use indexmap::IndexMap;
use slog_scope::{error, info};
use smithay::backend::drm::{DrmEvent, DrmNode, GbmBufferedSurface};
...
use smithay::reexports::drm::control::{crtc, Device as _, ModeTypeFlags};
    ...
    pub fn clear(&mut self, renderer: &mut DrmRenderer) -> Result<()> {
        ...
        renderer.render(
            ...
            |_, frame| {
                ...
                let rectangle = Rectangle::from_loc_and_size((0, 0), size);

                frame.clear([0.2, 0.2, 0.2, 1.0], &[rectangle])
            },
        )??;
